...
public class LogsServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (customer == null) {
            System.out.print("cust is null");
        }
        if (logsDAO == null) {
            System.out.print("logsdao is null");
        } 



        try {
            ArrayList<Logs> logs = logsDAO.fetchLogs(customer.getUserID());
            session.setAttribute("logs", logs);
            if (logs == null) {
            System.out.print("logs is null");
        } 
//            request.getRequestDispatcher("store.jsp").include(request, response);
            ...
            Logger.getLogger(LogsServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
}
